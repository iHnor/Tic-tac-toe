...
namespace tic_tac_toe
{
    class Program
    {
        ...
        static char[,] DoStep(char[,] field, int row, int col, char XorO)
        {
            if (XorO == 'x' || XorO == 'o')
            {
        ...
        static bool checkCell(int row, int col, char[,] field)
        {
            if (field[row, col] == ' ') return true;
            else return false;
        }
        static char isWin(char[,] field)
        {
            string isX = "xxx";
            string isO = "ooo";
        ...
        static bool isСellsAreFree(char[,] field)
        {
            for (int i = 0; i < 3; i++)
            {
                for (int j = 0; j < 3; j++)
        ...
        static bool isVertical(char[,] field, string XorO)
        {
            for (int i = 0; i < 3; i++)
            {
        ...
        static void ShowTheField(char[,] field)
        {
            string show = transformField(field);
            Console.WriteLine(show);
        }

        static string transformField(char[,] field)
        {
